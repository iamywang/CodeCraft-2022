...
int main()
{
    ...
    {
        printf("Time: %s, Streams: %d\n", global::g_demand.mtime[i].c_str(), global::g_demand.stream_client_demand[i].id_local_stream_2_stream_name.size());
        for (int j = 0; j < global::g_demand.stream_client_demand[i].id_local_stream_2_stream_name.size(); j++)
        {
            string stream_name = global::g_demand.stream_client_demand[i].id_local_stream_2_stream_name[j];
            printf("Stream %d %s: ", global::g_demand.stream_client_demand[i].stream_name_2_id_local_stream.find(stream_name)->second, stream_name.c_str());
            for (int k = 0; k < global::g_demand.client_name.size(); k++)
            {
                printf("%d ", global::g_demand.stream_client_demand[i].stream_2_client_demand[j][k]);
            }
            printf("\n");
        }
    }
    ...
    {
        X_results[i].stream2server_id.resize(global::g_demand.stream_client_demand[i].id_local_stream_2_stream_name.size());
